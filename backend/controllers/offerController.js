...
exports.createOffer = async (req, res) => {
  try {
    ...
    const { jobId, job: jobField, studentId, student, package, tier, offerDetails, joiningDate } = req.body;
    ...
    if (job.company.toString() !== companyProfile._id.toString()) {
      return res.status(401).json({ msg: 'Not authorized to create offers for this job' });
    }
    ...
    const existingOffer = await Offer.findOne({ job: jobId, student: studentId });
    if (existingOffer) {
      return res.status(400).json({ msg: 'An offer already exists for this student and job' });
    }
...
exports.getStudentOffers = async (req, res) => {
  try {
    ...
    if (req.user.role === 'student') {
      ...
      return res.status(403).json({ msg: 'Not authorized to view these offers' });
    }
    
    const offers = await Offer.find({ student: studentId })
      .populate({
        path: 'job',
        select: 'title description location jobType package applicationDeadline'
      })
      .populate({
        path: 'company',
...
exports.updateOfferStatus = async (req, res) => {
  try {
    ...
    if (!['Accepted', 'Rejected'].includes(status)) {
      return res.status(400).json({ msg: 'Status must be either Accepted or Rejected' });
    }
    ...
    if (offer.student.toString() !== studentProfile._id.toString()) {
      return res.status(401).json({ msg: 'Not authorized to update this offer' });
    }
    
    if (offer.status !== 'Pending') {
      return res.status(400).json({ msg: 'Cannot update an offer that is not pending' });
    }
...
exports.getOfferById = async (req, res) => {
  try {
    ...
    if (req.user.role === 'company') {
      const companyProfile = await CompanyProfile.findOne({ user: req.user.id });
      if (companyProfile && offer.company._id.toString() === companyProfile._id.toString()) {
        authorized = true;
      }
    }
